...
public class GuitarPane extends SheetPane {

    GuitarPane(Instrument instrument) {
        super(instrument);
    }
    ...
    protected VBox buildMeasures() {
        final VBox measures = new VBox();

        final HBox row = new HBox(0);

        for (Measure measure : part.getMeasures()) {
            row.getChildren().add(new StaffPane());
        }

        measures.getChildren().add(row);
